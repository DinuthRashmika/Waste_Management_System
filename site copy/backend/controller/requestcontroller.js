...
// Controller to assign a collector to a request
exports.assignCollector = async (req, res) => {
    try {
      const { requestId, collectorId } = req.body;
  
      // Find the request and update with assigned collector
      const request = await Request.findById(requestId);
      if (!request) {
        return res.status(404).json({ message: 'Request not found' });
      }
  
      // Find the collector (make sure the user exists and has the collector role)
      ...
      res.status(200).json({ message: 'Collector assigned successfully' });
    } catch (error) {
